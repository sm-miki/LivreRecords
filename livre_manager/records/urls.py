...
urlpatterns = [
	...
	path("acquisition/new", views.AcquisitionEditView.as_view(), name="acquisition_new"),  # 入手記録の新規作成
	...
	path("acquisition/<int:pk>/delete", views.acquisition_delete, name="acquisition_delete"),  # 入手記録削除のエンドポイント
	path("acquisition/receipt_ocr/", views.receipt_ocr, name="receipt_ocr"), # OCRエンドポイント
	path("book/", views.book_list, name="book_list"),  # 書籍一覧
	path("book/new", views.BookEditView.as_view(), name="book_new"),  # 書籍の新規登録
	...
	path("book/<str:pk>/delete", views.book_delete, name="book_delete"),  # 書籍削除のエンドポイント
